...
'''
10. 배열 파티션 : n개의 페어를 이용한 min(a,b)의 합으로 만들 수 있는 가장 큰 수
'''

# 추상 클래스 정의
class ThreeSumAlgorithm(ABC):
    # 추상 메서드 정의
    @abstractmethod
    # 리스트와 타깃을 입력, 리스트 출력
...
class TwoPointer(ThreeSumAlgorithm):
    def algorithm(self, nums: List[int]) -> List[List[int]]:
        results = []
        # 정렬
        nums.sort()
...
def run_tests():
    ...
    # 알고리즘 인스턴스 생성
    algorithms = get_algorithms(ThreeSumAlgorithm)
